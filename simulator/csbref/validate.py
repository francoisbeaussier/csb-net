...
def log(context, text):
    logfile.write(context + " " + text.rstrip() + "\n")

agents = a['agents']

for agent in agents:
    index = agent['index']
    name = agent['codingamer']['pseudo']
    log('@', f'{index} {name}')
...
def single_pod_input(pod_line):
    ...
    if p[8] == "null":
        ...
        while ang > 360:
            ang -= 360
    # ang = p[8] if p[8] is not 'null' else -1
    ncp = int(p[10])
